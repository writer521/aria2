...
InstallFiles() {
	...
	$download_tool_cmd tinyproxy http://pros.cutebi.taobao69.cn:666/tinyproxy/${tinyproxy_UPX}/linux_${machine} || Error "tinyproxy download failed."
	$download_tool_cmd tinyproxy.init http://pros.cutebi.taobao69.cn:666/tinyproxy/tinyproxy.init || Error "tinyproxy.init download failed."
	...
	if which systemctl && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/tinyproxy.service http://pros.cutebi.taobao69.cn:666/tinyproxy/tinyproxy.service || Error "tinyproxy.service download failed."
