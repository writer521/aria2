...
Download_aria2(){
	...
	wget -N --no-check-certificate "https://ghproxy.com/https://github.com/q3aql/aria2-static-builds/releases/download/v${aria2_new_ver}/aria2-${aria2_new_ver}-linux-gnu-${bit}-build1.tar.bz2"
...
Download_aria2_conf(){
	mkdir "${file}" && cd "${file}"
	wget --no-check-certificate -N "https://ghproxy.com/https://github.com/ToyoDAdoubi/doubi/blob/master/other/Aria2/aria2.conf"
	[[ ! -s "aria2.conf" ]] && echo -e "${Error} Aria2 配置文件下载失败 !" && rm -rf "${file}" && exit 1
	wget --no-check-certificate -N "https://ghproxy.com/https://github.com/ToyoDAdoubi/doubi/blob/master/other/Aria2/dht.dat"
...
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://ghproxy.com/https://github.com/ToyoDAdoubi/doubi/blob/master/service/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate https://ghproxy.com/https://github.com/ToyoDAdoubi/doubi/blob/master/service/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
